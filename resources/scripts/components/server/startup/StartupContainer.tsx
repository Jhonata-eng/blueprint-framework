...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Configurações de startup'} showFlashKey={'startup:image'}>
            <BeforeContent />
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Comando de startup'} css={tw`flex-1`}>
                    ...
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p css={tw`text-xs text-neutral-300 mt-2`}>
                                ...
                                executar esta instância do servidor.
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    A imagem Docker deste {"server's"} servidor foi definida manualmente por um
                                    administrador e não pode ser alterada por esta interface.
                                </p>
                            )}
                        </>
                    )}
